...
Describe "DiskSmartInfo remocking tests" {
    ...
    Context "ComputerName IP Address" {
        ...
        BeforeAll {
            $cimSessionHost3 = New-MockObject -Type 'Microsoft.Management.Infrastructure.CimSession' -Properties @{ComputerName = $ipAddress}
            mock New-CimSession -MockWith { $cimSessionHost3 } -ModuleName DiskSmartInfo
            mock Remove-CimSession -MockWith { } -ModuleName DiskSmartInfo
