...
Describe "DiskSmartInfo remoting SSHSession mocked Ctl" -Skip:(-not $IsCoreCLR) {
    ...
    Context "ComputerName" {
        ...
        BeforeAll {
            ...
            if (-not $IsLinux)
            {
                ...
                $diskSmartInfo = Get-DiskSmartInfo -ComputerName $computerNames -Transport SSHSession -Source SmartCtl
            }
            elseif ($IsLinux)
            {
    ...
    Context "ComputerName IP Address" {
        ...
        BeforeAll {
            ...
            if (-not $IsLinux)
            {
                ...
                $diskSmartInfo = Get-DiskSmartInfo -ComputerName $ipAddresses -Transport SSHSession -Source SmartCtl
            }
            elseif ($IsLinux)
            {
                mock Invoke-Command -MockWith { $true } -ParameterFilter { $ScriptBlock.ToString() -eq ' $IsLinux ' } -ModuleName DiskSmartInfo
    ...
    Context "ComputerName positional" {
        ...
        BeforeAll {
            ...
            if (-not $IsLinux)
            {
                ...
                $diskSmartInfo = Get-DiskSmartInfo $computerNames -Transport SSHSession -Source SmartCtl
            }
            elseif ($IsLinux)
            {
                mock Invoke-Command -MockWith { $true } -ParameterFilter { $ScriptBlock.ToString() -eq ' $IsLinux ' } -ModuleName DiskSmartInfo
    ...
    Context "ComputerName positional IP Address" {
        ...
        BeforeAll {
            ...
            if (-not $IsLinux)
            {
                ...
                $diskSmartInfo = Get-DiskSmartInfo $ipAddresses -Transport SSHSession -Source SmartCtl
            }
            elseif ($IsLinux)
            {
                mock Invoke-Command -MockWith { $true } -ParameterFilter { $ScriptBlock.ToString() -eq ' $IsLinux ' } -ModuleName DiskSmartInfo
    ...
    Context "ComputerName pipeline" {
        ...
        BeforeAll {
            ...
            if (-not $IsLinux)
            {
                ...
                $diskSmartInfo = $computerNames | Get-DiskSmartInfo -Transport SSHSession -Source SmartCtl
            }
            elseif ($IsLinux)
            {
                mock Invoke-Command -MockWith { $true } -ParameterFilter { $ScriptBlock.ToString() -eq ' $IsLinux ' } -ModuleName DiskSmartInfo
    ...
    Context "Win32_DiskDrive pipeline" -Skip:$IsLinux {
        ...
        BeforeAll {
            ...
            $diskSmartInfo = $diskDriveHost1, $diskDriveHost2 | Get-DiskSmartInfo -Transport SSHSession -Source SmartCtl
        }
        ...
        It "Has ComputerName, Model, and Device properties" {
            ...
            {
                $diskSmartInfo[0].DiskModel | Should -BeExactly $testDataCtl.CtlModel_HDD2
                $diskSmartInfo[0].Device | Should -BeExactly $testDataCtl.CtlDevice_HDD2
                $diskSmartInfo[0].SmartData | Should -HaveCount 18
            }
            elseif ($diskSmartInfo[0].ComputerName -eq $computerNames[1])
            {
                $diskSmartInfo[0].Model | Should -BeExactly $testDataCtl.CtlModel_SSD1
                $diskSmartInfo[0].Device | Should -BeExactly $testDataCtl.CtlDevice_SSD1
            ...
            {
                $diskSmartInfo[1].Model | Should -BeExactly $testDataCtl.CtlModel_HDD2
            ...
            {
                $diskSmartInfo[1].DiskModel | Should -BeExactly $testDataCtl.CtlModel_SSD1
    ...
    Context "MSFT_Disk pipeline" -Skip:$IsLinux {
        ...
        BeforeAll {
            ...
            $diskSmartInfo = $diskHost1, $diskHost2 | Get-DiskSmartInfo -Transport SSHSession -Source SmartCtl
        }
    ...
    Context "MSFT_PhysicalDisk pipeline" -Skip:$IsLinux {
        ...
        BeforeAll {
            ...
            $diskSmartInfo = $physicalDiskHost1, $physicalDiskHost2 | Get-DiskSmartInfo -Transport SSHSession -Source SmartCtl
        }
    ...
    Context "History ComputerName" {
        ...
        Context "-UpdateHistory" {
            ...
            BeforeAll {
                ...
                if (-not $IsLinux)
                {
                    Get-DiskSmartInfo -ComputerName $computerNames[0] -Transport SSHSession -Source SmartCtl -UpdateHistory | Out-Null
                }
                elseif ($IsLinux)
                {
            ...
            It "Historical data file contains proper data" {
                if ($IsCoreCLR)
                {
                    $filepath | Should -FileContentMatch ([regex]::Escape('"Device": "/dev/sda"'))
                }
                else
                {
                    $filepath | Should -FileContentMatch ([regex]::Escape('"Device":  "/dev/sda"'))
        ...
        Context "-ShowHistory" {
            ...
            BeforeAll {
                ...
                {
                    Get-DiskSmartInfo -ComputerName $computerNames[0] -Transport SSHSession -Source SmartCtl -UpdateHistory | Out-Null
                    $diskSmartInfo = Get-DiskSmartInfo -ComputerName $computerNames[0] -Transport SSHSession -Source SmartCtl -ShowHistory
                }
                elseif ($IsLinux)
                {
                    Get-DiskSmartInfo -ComputerName $computerNames[0] -UpdateHistory | Out-Null
    ...
    Context "SmartCtlOption" {
        ...
        BeforeAll {
            ...
            if (-not $IsLinux)
            {
                ...
                $diskSmartInfo = Get-DiskSmartInfo -ComputerName $computerNames -Transport SSHSession -Source SmartCtl -SmartCtlOption '-d ata'
            }
            elseif ($IsLinux)
            {
                mock Invoke-Command -MockWith { $true } -ParameterFilter { $ScriptBlock.ToString() -eq ' $IsLinux ' } -ModuleName DiskSmartInfo
                mock Invoke-Command -MockWith { $ctlDataHDD1 } -ParameterFilter { $ScriptBlock.ToString() -eq "sudo smartctl -d ata --info --health --attributes /dev/sda" } -ModuleName DiskSmartInfo

                $diskSmartInfo = Get-DiskSmartInfo -ComputerName $computerNames -SmartCtlOption '-d ata'
            }
        }
